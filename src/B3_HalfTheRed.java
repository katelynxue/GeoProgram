public class B3_HalfTheRed extends World {
    ...
    public void go() {
        ...
        for (int row = 1; row < 900; row = row + 1) {
            for (int col = 1; col < 900; col++) {
                ...
                if (red == 255 && green == 255 && blue == 255) {
                    plane.setPixelColor(3, 194, 255);
                } else if (red > 230 && red <= 255 && green > 230 && green <= 255 && blue > 230 && blue <= 255) {
                    plane.setPixelColor(3, 194, 255);
                } else if (red>=210 && red<=255 && green>=160 && green<=255 && blue>=0 && blue<=70) {
                    plane.setPixelColor(121, 119, 230);
                } else if (red>=240 && red<=255 && green>=230 && green<=255 && blue>=0 && blue<=200)
                    plane.setPixelColor(224,85,108);
            }
            if(red>200 && green>0 && green<50 && blue>0 && blue<60)
                plane.setPixelColor(23,230,140);
        }

        plane.teleport(790,90);
        plane.isTrail = true;
        plane.pausetime = 0;
        plane.circle(50);
        plane.teleport(846,190);
        plane.pausetime = 1;
        plane.move(100);
        plane.teleport(784,173);
        plane.startingAngle(125);
        plane.move(100);
        plane.teleport(746,111);
        plane.startingAngle(180);
        plane.move(100);
        plane.teleport(738,48);
        plane.startingAngle(205);
        plane.move(100);
        plane.teleport(906,174);
        plane.startingAngle(45);
        plane.move(100);
        plane.teleport(942,132);
